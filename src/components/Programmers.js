...
export default function Programmers() {
  ...
  const [featureProgrammer, setFeatureProgrammer] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    const [dev] = programmersList.filter(dev => {
     return dev.id === featureProgrammer
    }); 
    return dev.name
  };
      ...
      <div className='programmers'>
        {
          ...
          programmersList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setFeatureProgrammer(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
